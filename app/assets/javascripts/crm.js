...
var crm = {
  ...
  show_account_type_fields: function(value) {
  	if (value == '2') {
  		$("company_account").show();
  	} else {
  		$("company_account").hide();
  	}
  },
...
//----------------------------------------------------------------------------
  create_product: function() {
    if ($("product_business_address_attributes_country")) {
      this.clear_all_hints();
    }
    $("product_assigned_to").value = $F("product_assigned_to");
    if ($("product_id").visible()) {
      $("product_id").enable();
    }
  },
