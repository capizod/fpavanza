...
class Account < ActiveRecord::Base
  ...
  has_many    :products, :through => :account_products, :uniq => true, :order => "products.id DESC"
  ...
  scope :lonely, lambda { joins('LEFT OUTER JOIN account_products ON accounts.id = account_products.account_id').where('account_products.product_id IS NULL').order("name ASC") }

  scope :providerview, lambda { joins('LEFT OUTER JOIN account_products ON accounts.id = account_products.account_id').where('account_products.product_id IS NULL') }
  ...
  validates_presence_of :account_type_id, :message => :missing_account_type
